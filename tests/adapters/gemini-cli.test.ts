...
test('gemini-cli: id is "gemini-cli"', () => {
  assert.equal(adapter.id, 'gemini-cli');
});

test('gemini-cli: configDir points to ~/.gemini', () => {
  assert.equal(adapter.configDir, path.join(os.homedir(), '.gemini'));
});

test('gemini-cli: mapEventName maps PascalCase to internal names', () => {
...
test('gemini-cli: mapEventName returns null for unknown events', () => {
  assert.equal(adapter.mapEventName('UnknownEvent'), null);
  assert.equal(adapter.mapEventName('beforeSubmitPrompt'), null);
});

test('gemini-cli: normalizeInput coalesces transcript_path on stop', () => {
...
test('gemini-cli: normalizeInput routes afterToolUse Write to afterFileEdit', () => {
  const out = adapter.normalizeInput('afterToolUse', { tool_name: 'Write', file_path: '/tmp/f.ts' });
  assert.equal(out._routeTo, 'afterFileEdit');
});

test('gemini-cli: normalizeInput routes afterToolUse Edit to afterFileEdit', () => {
  const out = adapter.normalizeInput('afterToolUse', { tool_name: 'Edit' });
  assert.equal(out._routeTo, 'afterFileEdit');
});

test('gemini-cli: normalizeInput routes afterToolUse FileWrite to afterFileEdit', () => {
  const out = adapter.normalizeInput('afterToolUse', { tool_name: 'FileWrite' });
  assert.equal(out._routeTo, 'afterFileEdit');
});

test('gemini-cli: normalizeInput routes afterToolUse FileEdit to afterFileEdit', () => {
  const out = adapter.normalizeInput('afterToolUse', { tool_name: 'FileEdit' });
  assert.equal(out._routeTo, 'afterFileEdit');
});

test('gemini-cli: normalizeInput routes afterToolUse MultiEdit to afterFileEdit', () => {
  const out = adapter.normalizeInput('afterToolUse', { tool_name: 'MultiEdit' });
  assert.equal(out._routeTo, 'afterFileEdit');
});

test('gemini-cli: normalizeInput defaults unknown tool to afterShellExecution', () => {
  const out = adapter.normalizeInput('afterToolUse', { tool_name: 'Read' });
  assert.equal(out._routeTo, 'afterShellExecution');
});

test('gemini-cli: normalizeInput pass-through for other events', () => {
  const raw = { session_id: 's1' };
  const out = adapter.normalizeInput('sessionStart', raw);
  assert.deepEqual(out, raw);
});

test('gemini-cli: generateHooksConfig produces nested format with matcher', () => {
...
test('gemini-cli: generateHooksConfig uses cmd.exe on win32', () => {
...
test('gemini-cli: generateMcpConfig produces standard MCP structure', () => {
