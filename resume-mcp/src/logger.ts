 ...
 *
 * Adapted from agentMemory-hybrid-mcp/src/logger.ts (kept identical so behaviour
 * matches the sister MCP exactly).
 */
...
export const logger = {
  debug(msg: string, data?: unknown) {
    if (process.env.DEBUG || process.env.AGENTMEM_RESUME_DEBUG) {
